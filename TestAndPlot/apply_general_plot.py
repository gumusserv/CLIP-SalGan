...
generator.load_state_dict(torch.load('generator_model_final_general.pt', map_location = torch.device('cpu')))
discriminator.load_state_dict(torch.load('discriminator_model_final_general.pt', map_location = torch.device('cpu')))
...
with open('test_data_list_general.json', 'r') as f:
...
# image_paths_all = ['saliency/image_1800/000000015846_0.png','saliency/image_1800/000000015846.png']
# target_paths_all = ['saliency/map_1800/000000015846_0.png','saliency/map_1800/000000015846.png']
# print(len(test_data))
image_paths_all = []
target_paths_all = []
...
for i in range(0, len(test_data)):
    if '_1.png' in test_data[i][0]:
        image_paths_all.append(test_data[i][0].replace("_1.png", "_0.png"))

        target_paths_all.append(test_data[i][1].replace("_1.png", "_0.png"))
        image_paths_all.append(test_data[i][0])

        target_paths_all.append(test_data[i][1])
    elif "_0.png" not in test_data[i][0]:
        image_paths_all.append(test_data[i][0].replace(".", "_0."))

        target_paths_all.append(test_data[i][1].replace(".", "_0."))
        image_paths_all.append(test_data[i][0])
...
general_score_dic = dict()
general_score_dic['pure'] = {"AUC" : [], "sAUC" : [], "CC" : [], "NSS" : []}
general_score_dic['general'] = {"AUC" : [], "sAUC" : [], "CC" : [], "NSS" : []}
...
for i in range(0, len(image_paths_all), 2):
    ...
    for k in range(2):
        ...
        with torch.no_grad():
            ...
            for m, (images, targets, texts_embeddings) in enumerate(val_loader):
                ...
                if k == 0:
                    general_score_dic['pure']['AUC'].append(AUC_score)
                    general_score_dic['pure']['sAUC'].append(sAUC_score)
                    general_score_dic['pure']['CC'].append(CC_score)
                    general_score_dic['pure']['NSS'].append(NSS_score)
                elif k == 1:
                    general_score_dic['general']['AUC'].append(AUC_score)
                    general_score_dic['general']['sAUC'].append(sAUC_score)
                    general_score_dic['general']['CC'].append(CC_score)
                    general_score_dic['general']['NSS'].append(NSS_score)
    ...
    # fig.text(0.65, 0.85, 'Ground Truth', ha='center', va='center', fontsize=12, wrap=True, color = 'red')
    # fig.text(0.65, 0.15, 'Prediction', ha='center', va='center', fontsize=12, wrap=True, color = 'black')

    # fig.text(0.52, 0.5, 'Pure', ha='center', va='center', fontsize=12, wrap=True, color = 'green')
    # fig.text(0.77, 0.5, 'General Description:\nTwo boys playing frisbee\n at park near bench.', ha='center', va='center', fontsize=12, wrap=True, color = 'blue')
...
with open('general_score_dic.json', 'w') as file:
    json.dump(general_score_dic, file)
